...
public class CancelledReservationsPage extends JFrame implements Page {

    private static final long serialVersionUID = 1L;
    private Customer customer;
    private static final String COLOR = "red";

    public CancelledReservationsPage(CustomizedJPanel panel, Customer currentUser) {
        // Frame setup
    ...
    private void addComponentsToPanel(CustomizedJPanel panel) {
        ...
        JButton menuBarButton = CustomizedJButtons.createJPopupButtonsMenu("Menu", COLOR, reservationHistoryButton, customerMainPageButton, shutDownButton, logOutButton);
		...
		HashSet<Reservation> cancelledReservations = Reservation.getCanceledCustomerReservations(customer);
        // Append reservations to the text area
        if (cancelledReservations == null) {
            reservationsTextArea.append("There is no cancelled reservation for customer: " + customer.getName());
        }
        else {
    ...
    public Page createPage(CustomizedJPanel panel) {
        return new CancelledReservationsPage(panel, customer);
    }
}
